...
def _context_bundle_from_topic(req: TopicInitializationRequest) -> dict:
    return {
        ...
        "target_audience": req.target_audience.model_dump(),
...
def _load_topic_request(project_id: str, db: Session, user_id: str) -> TopicInitializationRequest:
    bundle = ProjectRepository(db, user_id=user_id).get_context_bundle(project_id)
    if not bundle:
    ...
    payload = {
        ...
        "voice_profile_id": bundle.get("voice_profile_id") or "",
...
def run_workflow(
    ...
    req = _load_topic_request(payload.project_id, db, user_id)
...
def run_research_node(
    ...
    req = _load_topic_request(project_id, db, user_id)
    engine = OrchestrationEngine(db, user_id=user_id)
...
def run_research_node_post(
    payload: ResearchNodeRunRequest,
    user_id: str = Depends(get_current_user_id),
    db: Session = Depends(get_db),
) -> ResearchTrendResponse:
    req = payload.topic
...
def run_master_node_post(
    payload: MasterNodeRunRequest,
    user_id: str = Depends(get_current_user_id),
    db: Session = Depends(get_db),
) -> MasterContentResponse:
    req = payload.topic
...
def run_editorial_node(
    payload: EditorialRequest,
    user_id: str = Depends(get_current_user_id),
    db: Session = Depends(get_db),
) -> EditorialResponse:
    try:
        return OrchestrationEngine(db, user_id=user_id).run_editorial(payload)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc


@router.post("/nodes/editorial/session/start", response_model=EditorialSessionStartResponse)
def start_editorial_session(
    payload: EditorialSessionStartRequest,
    user_id: str = Depends(get_current_user_id),
    db: Session = Depends(get_db),
) -> EditorialSessionStartResponse:
    engine = OrchestrationEngine(db, user_id=user_id)
...
def finalize_editorial_session(
    session_id: str,
    user_id: str = Depends(get_current_user_id),
    db: Session = Depends(get_db),
) -> EditorialSessionFinalizeResponse:
...
def generate_artifacts_for_latest_editorial(
