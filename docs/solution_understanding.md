...
6. Platform adapter outputs are generated by editorial paths that run post-editorial pipeline (for example: `/nodes/editorial`, session finalize, direct finalize). `finalize-selected` only marks final version.
